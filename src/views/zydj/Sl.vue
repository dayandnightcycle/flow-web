      ...
      <el-dialog v-model="dialogFormVisible" title="选择受理">
        <el-form :model="form" label-width="120px">
          <el-form-item label="下一个表单">
            <el-input v-model="input" placeholder="转移收费.json" />
            <!--<el-select
...
const getInputData = () => {
  ...
  axios.post("/test/sqls", data1).then((res) => {
    ...
    console.log(" proxy.newFormData2aaaaa",res.data.data);
       
    proxy.newFormData2 = formData;
    console.log('123',mm.qlfwxg==undefined)
    if(mm.qlfwxg.length!=0){
  ...
  proxy.newFormData2.fttdmj = proxy.newFormData.qltdxg[0].FTTDMJ;
   proxy.newFormData2.gytdmj = proxy.newFormData.qltdxg[0].GYTDMJ;
...
const getformJson = () => {
  ...
  proxy.$http
    ...
    .then((res) => {
      ...
      if(proxy.newFormData.ywrdlr.length!=0){
      ...
      proxy.newFormData2.YWRDLRZJHM = proxy.newFormData.ywrdlr[0].ZJHM;
        ...
        proxy.newFormData2.YWRDLRXH=''
      proxy.newFormData2.YWRDLRMC = '';
      proxy.newFormData2.YWRDLRZJZL = '';
      proxy.newFormData2.YWRDLRZJHM = '';
      }
      ...
      setTimeout(() => {
        ...
        chaxun.addEventListener("click", function () {
          ...
          const data4 = {
            fwxxcx:
              "SELECT FWBH,ZH,LJZH,XMMC,ZCS,DSCS,DXCS,FWJG,GHYT,FWZL,TSTYBM,ZDTYBM FROM FC_Z_QSDC WHERE (LIFECYCLE = 0 OR LIFECYCLE IS NULL)  AND ZH LIKE '%" +
              proxy.zrzh1 +
              "%'  AND XMMC LIKE '%" +
              proxy.xmmc1 +
              "%' AND FWZL LIKE '%" +
...
const conserve = () => {
  ...
  var data3 = {
    djtsgl:
      "insert into dj_tsgl (glbm,slbh,bdclx,tstybm,bdcdyh,djzl,cssj,lifecycle)values(to_char(SYSDATE,'yyyyMMddhhmiss')||'-'|| dbms_random.string('x', 19),'" +
      slbh2 +
      "','" +
      bdclx +
      "','" +
    ...
    djdjb:
      ...
      "'",
    qltdxg:
      "insert into ql_tdxg (qlbh,slbh,qllx,qlxz,syqx,qsrq,zzrq,tdyt,tdsyqr,gytdmj,dytdmj,fttdmj,jzzdmj,jzwzdmj,tdytms)values(to_char(SYSDATE,'yyMMddhh24miss')||'-'|| dbms_random.string('x', 19),'" +
      slbh +
      "','" +
      qllx +
      "','" +
      ...
      "')",
    qlfwxg:
      "insert into ql_fwxg (qlbh,slbh,qllx,qlxz,jzmj,tnjzmj,ftjzmj,qdfs,qdjg,jzmjms,ghytms,ghyt,pgje,jzwlx,gzwlx) values (to_char(SYSDATE,'yyMMddhh24miss')||'-'|| dbms_random.string('x', 19),'" +
      slbh +
      "','" +
      qllx +
      "','" +
