            ...
            <el-dialog v-model="dialogFormVisible" title="选择受理">
                <el-form :model="form" label-width="120px">
                    <el-form-item label="下一个表单">
                        <el-select v-model="form.namedata" @change="databd" placeholder="gzdj-fz.json">
                            <!-- <el-option
...
var data = {
    ...
    djdjb: "select SLBH, DJLX, DJYY, SQZSBS, SQFBCZ, SQRQ, SQNR, SQBZ, DLJGMC, DLRXM, DLRZJLX, DLRZJH, DLRZGZH, DLRDH, DLJGMC2, DLRXM2, DLRZJLX2, DLRZJH2, DLRZGZH2, DLRDH2, SPDW, SPRQ, SPBZ, BDCZH, SSJC, JGJC, FZND, ZSH, SZR, GYFS, GYFE, DJRQ, DBR, ZSR, FZJG, FZRQ, ZSLX, ZSXLH, DYCS, GDH, DAMJ, QT, FJ, XGZH, BDCDYH, LIFECYCLE from DJ_DJB where SLBH ='" +
        slbh +
        "'",
    qlfwxg: "select QLBH,SLBH,QLLX,QLXZ,JZMJ,TNJZMJ,FTJZMJ,QDJG,QDFS,JZMJMS,GHYTMS,QTNRMS,XCCJMJ,GCCJMJ,WZNF,WZJZMJ,GHYT from QL_FWXG where SLBH = '" +
        slbh +
        "'",
    qltdxg: "select QLBH,SLBH,QLLX,QLXZ,SYQX,QSRQ,ZZRQ,TDYT,TDSYQR,GYTDMJ,DYTDMJ,FTTDMJ,JZZDMJ from QL_TDXG where SLBH = '" +
        slbh +
        "'",
    qlrgl: "select b.zjlb,a.qlrlx,a.qlrmc,a.zjhm from dj_qlrgl a left join dj_qlr b on a.qlrid=b.qlrid where a.slbh ='" +
        slbh +
        "'",
    ggmjdwjxswskz: "select ITEMVAL,ITEMNOTE from DIC_ITEM where DICCODE = (SELECT DICCODE FROM DIC_MAIN WHERE DICNAME = '面积单位及小数位数控制' AND ROWNUM < 2) AND ITEMNAME = '房屋（建筑物）所有权' AND ROWNUM < 2",
    ggfwqlxz: "select itemval,itemname from dic_item where diccode = (select diccode from dic_main where dicname='房屋权利性质' and dictype='通用字典') order by itemorder",
    ggfwqdfs: "select itemval,itemname from dic_item where diccode = (select diccode from dic_main where dicname='房屋取得方式' and dictype='通用字典') order by itemorder",
    ggtdqllx: "select itemval,itemname from dic_item where diccode = (select diccode from dic_main where dicname='土地权利类型' and dictype='通用字典') order by itemorder",
    ggtdqlxz: "select itemval,itemname from dic_item where diccode = (select diccode from dic_main where dicname='土地权利性质' and dictype='通用字典') order by itemorder",
    ggfwqllx: "select itemval,itemname from dic_item where diccode = (select diccode from dic_main where dicname='房屋权利类型' and dictype='通用字典') order by itemorder",
    ggdjlxdl: "select itemval,itemname from dic_item where diccode = (select diccode from dic_main where dicname='房屋权利类型' and dictype='通用字典') order by itemorder",
    ggdjxl: "select ITEMNOTE,ITEMNAME from DIC_ITEM where DICCODE = (SELECT DICCODE FROM DIC_MAIN WHERE DICNAME = '国有建设用地使用权及房屋所有权转移登记' AND ROWNUM<2) ORDER BY ITEMORDER",
    ggxgxsxz: "select itemval from dic_item where diccode = (select diccode from dic_main where dicname='限购限售控制' and dictype='通用字典') and itemname ='限售' order by itemorder",
};
//表单数据获取
const getInputData = () => {
...
const getformJson = () => {
    proxy.$http
        .get("/flow/formJson?taskName=sz&taskId=" + taskId)
        .then((res) => {
...
const confirm = () => {
    proxy.$http
        .get(
            ...
            taskId +
            "&taskName=sz&users=fzusers&user=" +
            proxy.ID +
            "&name=gzdj-sz.json"
            // +
            // proxy.dataxz
...
const conserve = () => {
    ...
    //qlfwxg
    var data3 = {
        djtsgl: "UPDATE DJ_TSGL SET LIFECYCLE=0 WHERE SLBH='" +
            slbh +
            "' AND LIFECYCLE=-1",
        djdjb: "update dj_djb set fzjg='" +
            fzjg +
            "',szr='" +
            szr +
            "',dbr='" +
