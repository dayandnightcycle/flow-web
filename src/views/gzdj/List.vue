<template>
  <div>
    <h1>更正登记</h1>
    <!-- <el-button class="logout" id="logout-button" type="primary" @click="logout"
      >退出</el-button
    > -->
    <el-button type="primary" @click="startGzdj">发起流程</el-button>
    <div class="tasks">
      <h1>待处理任务</h1>
...
<div>
    <el-pagination v-model:page-size="page.size" :page-sizes="[10, 20, 30, 40]" layout="total, sizes, prev, pager, next, jumper" :total="Number(page.total || 0)" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
</div>
</div>
</div>
</template>
...
<script setup>
    ...
    const startGzdj = () => {
        proxy.$http.post("/test/slbh").then((res) => {
            ...
            proxy.$http
                .post(
                    "/flow/GZDJ/qd2?key=GZDJHZM&user=" +
                    userId +
                    "&name=gzdj-sj.json&slbh=" +
                    slbh
                )
                .then((res) => {
    ...
    const claim = (taskId) => {
        // console.log(taskId);
        proxy.$http
            .post("/flow/GZDJ/qsrw?taskId=" + taskId + "&userId=" + userId)
            .then((res) => {
                const code = res.data.code;
                if (code === 0) {
                    ElMessage({
    ...
    const handle = (task) => {
        ...
        } else if (task.name == "缮证") {
            store.dispatch("gblc", "sz");
        ...
        proxy.$http.post("/test/qtbm").then((res) => {
            ...
            proxy.$http
                ...
                .then((res) => {
                    router.push({
                        path: "/gzdj" + "/" + getUrlByName(task.name),
                        query: {
                            id: task.id,
                            slbh: description,
                            qlrid: qlrid,
    ...
    const handleCurrentChange = (val) => {
        ...
        proxy.$http
            .get(
                "/flow/GZDJ/cscx2?userId=" +
                userId +
                "&processDefinitionKey=GZDJHZM" +
                "&pageNumber=" +
                pageNumber +
                "&pageSize=" +
                pageSize
