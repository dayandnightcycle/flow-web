      ...
      <el-dialog v-model="dialogFormVisible" title="选择收件">
        <el-form :model="form" label-width="120px">
          <el-form-item label="下一个表单">
            <el-input v-model="input" placeholder="gzdj-sl.json" />
            <!-- <el-select
...
<script setup>
    ...
    var data = {
        djsjd: "select SLBH,QXKZ,LCLX, LCMC, SJR, SJSJ, CNSJ, QXDM, TZRXM, TZFS, TZRDH, TZRYDDH, TZRDZYJ, TZRTXDZ, CXMM, SJMC, SJLX, SJSL, YS, SFSYBZ, SFEWSJ, SFBCSJ, ZRKS,ZRR,YXJ,SJBZ,BCSM,DJXL,FCSLH,FCDJLX,JYBH,FCHID,DJDL,ZL,HTBH,PCH,PRJID,XCZP,LCLB from DJ_SJD where SLBH = '" +
            SLBH +
            "'",
        djdjb: "select SLBH,SQFBCZ,GYFS,DJLX,DJYY,DJRQ,SQRQ,XGZH,LIFECYCLE from DJ_DJB where SLBH = ''",
                            ...
                            document.addEventListener("click", function(e) {
                                ...
                                xgzhqr.addEventListener("click", function() {
                                    ...
                                    setTimeout(() => {
                                        ...
                                        for (let i = 0; i < array.length; i++) {
                                            xgzhspl = {
                                                ywrxx: "SELECT A.QLRMC,A.ZJLB,A.ZJHM,A.DH,B.QLRID,B.GYFE FROM DJ_QLR A INNER JOIN DJ_QLRGL B ON A.QLRID = B.QLRID WHERE B.SLBH IN ('" +
                                                    array[i][2] +
                    ...
                    nextTick(() => {
                        setTimeout(() => {
                            ...
                            chaxun.addEventListener("click", function() {
                                ...
                                const data3 = {
                                    qlrxx: "SELECT DISTINCT N'' AS XZ,ROWNUM AS RN,QLRMC,bdcdj.f_ParseDic('证件类型',ZJLB)as ZJLB,ZJHM,DH, QLRID,FRDBXM,FRDBZJH FROM DJ_QLR WHERE ZJHM IS NOT NULL AND QLRMC LIKE '%" +
                                        proxy.qlrmc +
                                        "%'AND ZJHM LIKE'%" +
                                        proxy.zjbm +
                                        "%'",
    ...
    const conserve = () => {
        ...
        for (let i = 0; i < XGZHXX1.length; i++) {
            ...
            var data2 = {
                djxgdjgl: "insert into dj_xgdjgl(bgbm,zslbh,fslbh,bgrq,bglx,xgzh,xgzlx)values('GL-'||to_char(SYSDATE,'yyMMddhhmiss')||'-'||dbms_random.string('x',10),'" +
                    slbh +
                    "','" +
                    XGZHXX1[i][2] +
                    "','" +
        ...
        //登记簿、收件单
        let datasj = {
            djsjd: "insert into dj_sjd (slbh,djdl,djxl,lclx,lcmc,zl,sjr,sjsj,cnsj,qxdm,tzrxm,tzfs,tzrdh,tzryddh,tzrdzyj,cxmm,zrks,yxj,sjbz,prjid,qxkz,cgzt,lclb)values('" +
                slbh +
                "','" +
                djdl +
                "','" +
                ...
                lclb +
                "')",
            djdjb: "insert into dj_djb(slbh,djlx,djyy,sqfbcz,sqrq,gyfs,xgzh)values('" +
                slbh +
                "','" +
                djlx +
                "','" +
                ...
                XGZHXX1[0][5] +
                "')",
            wfmprocinst: "insert into wfm_procinst(prjid,querypwd,procid,procname,prjname,prjtype,acceptby,accepttime,priority,prjlimit,limittime,exepolicy,regionpolicy,popedom,proposer,prjstate,initaddr)values('" +
                slbh +
                "','" +
                querypwd +
                "','" +
                ...
                initaddr +
                "')",
            wfmactinst: "insert into wfm_actinst(wrkid,prjid,mdltype,mdlid,stepname,submitby,submittime,acceptby,accepttime,savetime,submittype,completetime,stepstate,steptype,stepnum,steplimit,biztype)values('" +
                wrkid +
                "','" +
                slbh +
                "','" +
                ...
                steptype +
                "')",
            wfmexecutor: "insert into wfm_executor(exeid,exetype,executor,parenttype,parentnode,parentstate,how2exe,grantway,sortnum)values('" +
                exeid +
                "','" +
                exetype +
                "','" +
        ...
        for (let i = 0; i < QLRXX1.length; i++) {
            axios.post("/test/glbm").then((res) => {
                ...
                let gyfe = "";
                let data20 = {
                    djqlrgl: "insert into dj_qlrgl(glbm,slbh,qlrid,gyfs,sxh,qlrlx,qlrmc,zjlb,dh,czfs,gyfe)values('" +
                        qlrglbm +
                        "','" +
                        slbh +
                        "','" +
        ...
        for (let i = 0; i < QLRDLRXX1.length; i++) {
            axios.post("/test/glbm").then((res) => {
                ...
                let gyfe = "";
                let data21 = {
                    djqlrdlgl: "insert into dj_qlrgl(glbm,slbh,qlrid,gyfs,sxh,qlrlx,qlrmc,zjlb,dh,czfs,gyfe)values('" +
                        qlrdlrglbm +
                        "','" +
                        slbh +
                        "','" +
        ...
        for (let i = 0; i < YWRXX1.length; i++) {
            axios.post("/test/glbm").then((res) => {
                ...
                let gyfe = "";
                let data22 = {
                    djywrgl: "insert into dj_qlrgl(glbm,slbh,qlrid,gyfs,sxh,qlrlx,qlrmc,zjlb,dh,czfs,gyfe)values('" +
                        ywrglbm +
                        "','" +
                        slbh +
                        "','" +
        ...
        for (let i = 0; i < YWRDLRXX1.length; i++) {
            axios.post("/test/glbm").then((res) => {
                ...
                let gyfe = "";
                let data23 = {
                    djywrdlrgl: "insert into dj_qlrgl(glbm,slbh,qlrid,gyfs,sxh,qlrlx,qlrmc,zjlb,dh,czfs,gyfe)values('" +
                        ywrdlrglbm +
                        "','" +
                        slbh +
                        "','" +
