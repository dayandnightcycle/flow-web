      ...
      <el-dialog v-model="dialogFormVisible" title="选择收件">
        <el-form :model="form" label-width="120px">
          ...
          <el-form-item label="下一个表单">
            <span class="bd" style="margin-left: 10px">查封归档.json</span>
            <!-- <el-select
...
const getlcmc = () => {
  proxy.$http
    ...
    .then((res) => {
      console.log(",,", res);
      lclist.value = res.data.data[0].userName;
      console.log("dbr", lclist, "jg", res.data.data[0].userName);
    });
};
...
const getInputData = () => {
  ...
  axios.post("/test/sqls", data).then((res) => {
   ...
   console.log('nvdk',proxy.newFormData2)
...
const conserve = () => {
  ...
  let biztype = "";
  let mdltype = "";
  let dbr = document.getElementsByClassName("el-input__inner")[23].value;
  let djsj = document.getElementsByClassName("el-input__inner")[24].value;
  //初审点提交写入sql
  var data2 = {
    wfmactinst1:
    ...
    wfmactinst2:
      ...
      "' and stepname='收件'",
    dbr_update:
      "update dj_cf set dbr='" +
      dbr +
      "',djsj=to_date(to_char(sysdate,'yyyy/MM/dd hh24:mi:ss'),'yyyy/MM/dd hh24:mi:ss') where slbh='" +
      slbh +
      "'",
  };
  let config = {
